...
function discoverAvailableGames(): Set<string> {
  ...
  for (const g of ['ct01', 'ct03', 'ft09', 'gw01', 'gw02', 'ls20', 'vc33', 'ws03', 'ws04']) {
    games.add(g);
  }
...
export function validateEvalConfig(config: EvalConfig): ValidationResult {
  ...
  if (config.maxSteps < 1 || config.maxSteps > 10000) {
    errors.push({ field: 'maxSteps', message: `maxSteps must be 1-10000, got ${config.maxSteps}` });
  }
  if (config.numRuns < 1 || config.numRuns > 100) {
    errors.push({ field: 'numRuns', message: `numRuns must be 1-100, got ${config.numRuns}` });
  }
  if (config.contextWindow < 1 || config.contextWindow > 200) {
    errors.push({ field: 'contextWindow', message: `contextWindow must be 1-200, got ${config.contextWindow}` });
  }
  if (config.retryAttempts < 0 || config.retryAttempts > 100) {
    errors.push({ field: 'retryAttempts', message: `retryAttempts must be 0-100, got ${config.retryAttempts}` });
  }
  if (config.retryBackoffBase <= 0) {
    errors.push({ field: 'retryBackoffBase', message: 'retryBackoffBase must be positive' });
  }
  if (config.retryMaxWait <= 0) {
    errors.push({ field: 'retryMaxWait', message: 'retryMaxWait must be positive' });
  }
 ...
 */
export function validateSessionConfig(config: EvalSessionConfig): ValidationResult {
  const errors: ValidationError[] = [];
  const warnings: ValidationError[] = [];

  // Validate game IDs against discovered games (environment_files/ + arcengine)
  if (config.gameIds.length === 0) {
    errors.push({ field: 'gameIds', message: 'At least one game ID is required' });
  }
  ...
  if (config.modelKeys.length === 0) {
    errors.push({ field: 'modelKeys', message: 'At least one model key is required' });
  }
  ...
  if (config.maxSteps < 1) {
    errors.push({ field: 'maxSteps', message: 'maxSteps must be >= 1' });
  }
  if (config.contextWindow < 1) {
    errors.push({ field: 'contextWindow', message: 'contextWindow must be >= 1' });
  }
  ...
  if (config.withImages) {
    warnings.push({
      field: 'withImages',
      message: 'withImages=true has no effect for ARC3 games (renderPngBase64 returns null). Flag will be ignored.',
    });
  }
